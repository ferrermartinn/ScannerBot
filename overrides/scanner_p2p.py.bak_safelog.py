...
def build_asset_view(asset: str, cfg: dict) -> dict:
    ...
    sr = (f"{spread_percent:.3f}%" if spread_percent is not None else "NA")`n    se = (f"{eff_spread:.3f}%" if eff_spread is not None else "NA")`n    sr = f"{spread_percent:.3f}%" if spread_percent is not None else "NA"
se = f"{eff_spread:.3f}%" if eff_spread is not None else "NA"
log.info(f"[P2P] spread_raw={sr}  effective={se}  use_effective={cfg.get('spread_use_effective', True)}")
