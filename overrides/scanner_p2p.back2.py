...
def build_asset_view(asset: str, cfg: dict) -> dict:
    ...
    log.info(f"[P2P] spread_raw={(f"""{spread_percent:.3f}%""" if spread_percent is not None else ""NA")}  effective={(f"""{eff_spread:.3f}%""" if eff_spread is not None else ""NA")}  use_effective={cfg.get('spread_use_effective', True)}")
