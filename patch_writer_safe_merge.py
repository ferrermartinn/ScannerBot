import os, re, sys, json, py_compile
...
def patch_file(py: Path) -> bool:
    ...
    # 3) os.replace('...tmp','/app/data/data.json') -> comentar y delegar al helper si aparece un dumps previo
    s = re.sub(
        r"os\\.replace\\([^\\)]*['\\\"]/app/data/data\\.json['\\\"][^\\)]*\\)",
        r"# replaced_by_safe_merge_write", s
    )

    if need_import and "from _merge_helper import safe_merge_write" not in s:
        lines = s.splitlines()
        ins = 0
...
def main():
    ensure_helper()
    touched = []
    # Solo en /app/writer y /app/scripts del servicio writer
    for base in (ROOT / "writer", ROOT / "scripts"):
